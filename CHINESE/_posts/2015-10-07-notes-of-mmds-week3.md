...
计算过程种最好也Normalized：则Term i 在文档j 中的词频$f_{ij}$ ，总文档数N，则有：

\`T\_(ij)= f_(ij) / (max_k f\_(kj)),\` 
\`IDF_i = log(N/n_i),\`
\`TF-IDF\_(ij)=TF\_(ij) * IDF_j\`

其中$n_i$ 为term i 出现的文档数。

#### Cosine distance

通常

\`cos(theta) = (vec x * vec i)/(|vec x| * |vec i|)\`
...
其中，$r\_{xi}$ 是用户x 对i 的评分；搜 $s\_{ij}$ item i,j 两者的相似度。而$N(i;x)$ 则为用户x 评论过的跟i 相似的集合，而
