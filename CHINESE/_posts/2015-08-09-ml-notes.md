...
而优化过程同样可以像线性回归那样使用梯度下降求解，但需要随机初始化比较小的$vec x , vec theta$ 。其中$n_u$ 和 $n_m$ 分别代表用户和电影的数量。
...
\`
而若要让$theta$ 最终收敛，则在迭代过程中就需要逐步减小$alpha$
 
而mini-batch gradient descent 就是介于前两者之间，通常一次性计算2-100 个样本的平均梯度然后更新$\vec theta$
