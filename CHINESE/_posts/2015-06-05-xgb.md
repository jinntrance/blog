...
那么GBT 中我们要最小化目标函数，假设\`y_i^(k)\` 相形于 \`y_i^(k-1)\` 更靠近极值点，则，
{:.new_page]}

\`y_i^(k) = y_i^(k-1) - rho g_(y_i)\` 

接下来找新的一棵树$T_k$，使得 \`sum_(i=1)^N(-g\_(y_i) - T_k(x_i))^2\` 最小；得到 \`f_k= rho T_k\`


而进一步的看，因为有\`-ab>= -(a+b)^2 /2 \`
{:.hide}

则\`l(y_i^(k)) = l(y_i^(k-1)) - Delta y_i g_(y_i) = l(y_i^(k-1) + f_k(x_i)) >= l(y_i^(k-1)) - (Delta y\_i + g\_(y_i))^2/2\` 那么loss function 收敛的时候，\`Delta y + g\_(y_i)\ = 0\`，即 \` Delta y_i = f_k(x_i) = - g\_(y_i)\`
{:.hide}
...
树的学习结果，$\vec w$ 代表所有的叶子节点值的向量，设 q 函数将$x_i$ 映射到叶子节点j:
 $q(x_i)=j$，叶子节点集合设为 \`I_j = {i | q(x_i)=j}\` 则
 \`f_k(x_i) = w_(q(x_i))(x_i)\`
{:.new_page}
...
其实他们就分别代表：划分到某个叶子j 上的$l(y_i)$ ，对应的一阶、二阶导数值之和。
